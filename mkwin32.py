...
def compile_module(pyver):
    ...
    vars = locals()
    vars.update(globals())
    cmd = '%(CC)s -mno-cygwin %(OPT)s -mdll -DMODULE_NAME=\\"pysqlightning._sqlightning\\" -DSQLITE_ENABLE_RTREE=1 -DSQLITE_ENABLE_FTS3=1 -I amalgamation -I %(INC)s -I . %(SRC)s -L %(CROSS_TOOLS)s/python%(VER)s/libs -lpython%(VER)s -o build/%(LIBDIR)s/pysqlightning/_sqlite.pyd' % vars
    execute(cmd)
    execute("%(STRIP)s build/%(LIBDIR)s/pysqlightning/_sqlite.pyd" % vars)
...
def main():
    ...
    for ver in ["2.5", "2.6", "2.7"]:
        ...
        os.unlink("build/%(LIBDIR)s/pysqlightning/_sqlite.so" % vars)
